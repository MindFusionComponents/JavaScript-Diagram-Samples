...
document.addEventListener("DOMContentLoaded", function ()
{
    ...
    diagram.addEventListener("nodeDomCreated", function (sender, args)
    {
        ...
        var checkbox = node.getContent().querySelector("input[type='checkbox']");
        if (checkbox)
        {
...
data-interactive="true" data-event-change="dataChange"></input>`);
        ...
        var node2 = new MindFusion.Diagramming.ControlNode(diagram);
...
data-interactive="true" data-event-change="dataChange"></input>`);
        ...
        var node3 = new MindFusion.Diagramming.ControlNode(diagram);
...
function tasksMouseUp(e, sender)
{
    ...
    var node = nodeList.getDraggedNode();
    if (node)
    {
        var clone = node.clone();
        var bounds = sender.getBounds().clone();
        bounds.x += bounds.width + 10;
        bounds.width = 20;
        bounds.height = 40;
        clone.setBounds(bounds);
        sender.parent.addItem(clone);
        var link = sender.parent.getFactory().createDiagramLink(sender, clone);
