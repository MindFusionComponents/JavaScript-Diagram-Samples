...
namespace Flowcharter
{
	...
	import FontStyle = MindFusion.Drawing.FontStyle;
	...
	import ChangeItemCommand = MindFusion.Diagramming.ChangeItemCommand;
	import Events = MindFusion.Diagramming.Events;
	import Diagram = MindFusion.Diagramming.Diagram;
	import Overview = MindFusion.Diagramming.Overview;
	...
	declare var flowcharter_fc;
	declare var flowcharter_sample;
	...
	function onNodeSelected(sender, args)
	{
		...
		if (node && node.getStyle() !== undefined)
		{
			...
			if (style.getFontSize())
				...
				$('#fontSize').val('3');
			($("#fontName") as any).selectmenu("refresh");
			($("#fontSize") as any).selectmenu("refresh");
		}
		else
		{
			$('#fontName').val('Verdana');
	...
	export function onLinkShapes()
	{
		diagram.setBehavior(Behavior.LinkShapes);
	}

	export function onLinkTables()
	{
	...
	export function onSaveClick()
	{
		if (!localStorage)
			return;
		var name = ($('#tbFileName')[0] as HTMLInputElement).value;
